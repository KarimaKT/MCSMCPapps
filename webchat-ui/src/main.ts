...
const webchatEl = document.getElementById('webchat') as HTMLDivElement;

function setStatus(text: string, state: 'ok' | 'info' | 'error' = 'info'): void {
  if (!statusEl) return;
...
async function bootstrap(): Promise<void> {
  ...
  if (sso.source === 'none') {
    setStatus(
      'No Entra credentials configured — using anonymous Direct Line. Configure VITE_AAD_* to enable SSO.',
      'info'
    );
  } else {
  ...
  setTimeout(() => {
    if (statusEl.dataset.state === 'ok') statusEl.style.display = 'none';
  }, 2000);
}
