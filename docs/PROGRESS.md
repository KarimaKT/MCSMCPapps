...
| 5 | MCP App tool + DA manifest | 🔄 In progress | Confirmed mechanism: MCP server-backed UI widgets per [Learn](https://learn.microsoft.com/microsoft-365/copilot/extensibility/declarative-agent-ui-widgets). Auth flow doc + Architecture Center draft committed. Next: scaffold MCP server (Azure Function), DA manifest via Agents Toolkit, sideload to CDX. |
